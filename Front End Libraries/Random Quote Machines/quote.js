const quotes = [  
   ...
   {  
      "text":"Believe that you will succeed - and you will.",
      "from":"Dale Carnegie"
   },
   {  
   ...
   {  
      "text":"Just build things and find out if they work.",
...
class QuoteApp extends React.Component {
  constructor(props) {
	...
	//Methods
	getNewQuote() {
		let replacingQuote = newQuote();
		this.setState({quote: replacingQuote["text"],
									author: replacingQuote["from"]});
	}
  ...
  render () {
    return (
      <div id="quote-box"> 
				...
				<Buttons newQuote={this.getNewQuote} quote={this.state.quote} author={this.state.author}/>
			</div>
    )
  }
}
	...
	render() {
		return (
			<div id="buttons" style={{display:"flex", justifyContent: "space-between"}}>
				<a id="tweet-quote" href={`https://twitter.com/intent/tweet?text="${this.props.quote}" -${this.props.author}`} target="_blank">Tweet</a>
...
ReactDOM.render(<QuoteApp />, document.getElementById("container"));
